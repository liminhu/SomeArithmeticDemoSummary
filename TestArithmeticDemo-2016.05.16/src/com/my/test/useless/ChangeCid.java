...
public class ChangeCid {
	...
	public ChangeCid(String cid) {
		this.cid = cid;
	}
	
	public String cidToOtherString(){
		int v1; // r6@1
		int v2; // r12@1
		String v3; // r3@1
		int v4; // r5@1
		...
		int begin = 0;
		int end = 4;
		String temp=stringToFourByteHexString(v3.substring(begin, end));
		end +=4;
		
	//	String test = Integer.toUnsignedLong(arg0)
	//	String test = Integer.toHexString(arg0)
		
		long t =(v5 + changeStringToNumber(temp) - 680876936 + ((v2 ^ v1) & v4 ^ v1));
		v7 = lrotr(t, 25);
		v8 = v7 + v4;
		temp = stringToFourByteHexString(v3.substring(begin + 4, end));
		end +=4;
		v9  = changeStringToNumber(temp);
		t=v1 + v9 - 389564586 + ((v2 ^ v4) & v8 ^ v2);
		v10 = lrotr(t, 20);
		v11 = v10 + v8;
		temp = stringToFourByteHexString(v3.substring(begin + 8, end));
		end +=4;
		v12 =  changeStringToNumber(temp);
		Integer.valueOf(temp);
		v13 = lrotr(((v8 ^ v4) & v11 ^ v4) + v12 + v2 + 606105819, 15);
		v14 = v13 + v11;
		temp = stringToFourByteHexString(v3.substring(begin + 12, end));
		end +=4;
		v15 =  changeStringToNumber(temp);
		v16 = lrotr(((v11 ^ v8) & (v13 + v11) ^ v8) + v4 + v15 - 1044525330, 10);
		temp = stringToFourByteHexString(v3.substring(begin + 16, end));
		end +=4;
		v17 =  changeStringToNumber(temp);
		v18 = v16 + v13 + v11;
		v19 = lrotr((((v13 + v11) ^ v11) & v18 ^ v11) + v17 - 176418897 + v8, 25);
		temp = stringToFourByteHexString(v3.substring(begin + 20, end));
		end +=4;
		v20 =  changeStringToNumber(temp);
		v21 = v19 + v18;
		v22 = lrotr(((v18 ^ v14) & v21 ^ v14) + v20 + 1200080426 + v11, 20);
		temp = stringToFourByteHexString(v3.substring(begin + 24, end));
		end +=4;
		v23 =  changeStringToNumber(temp);
		v24 = v22 + v21;
		v25 = lrotr(((v21 ^ v18) & v24 ^ v18) + v23 - 1473231341 + v14, 15);
		temp = stringToFourByteHexString(v3.substring(begin + 28, end));
		end +=4;
		v26= changeStringToNumber(temp);
		v27 = v25 + v24;
		...
		result.append(getNeedStringFromInt(v5));
		result.append(getNeedStringFromInt(v4));
		result.append(getNeedStringFromInt(v2));
		result.append(getNeedStringFromInt(v1));
	/*	byte [] a = new byte[4];
		a[3] = (byte) ((v1 >> 24) & 0xff);
		a[2] = (byte) ((v1 >> 16) & 0xff);
		a[1] = (byte) ((v1 >> 8) & 0xff);
	...
	private String getNeedStringFromInt(int value){
		String needStr = Integer.toHexString(value);
		return getNewOrderString(needStr);
	}
	
	
	
	private int lrotr(long unsigned , int shift){
		 unsigned=getUnsignedLong(unsigned);
	 ...
	 * 重新排序一下输出原串
	 */
	private String getNewOrderString(String str) {
		...
		for (int i = length - 1; i >= 0; i--) {
			if (i % 2 == 0) {
				charArray[length - i - 2] = str.charAt(i);
			} else {
				charArray[length - i] = str.charAt(i);
			}
		}
	 ...
	 * @return
	 */
	private  int changeStringToNumber(String str) {
		int count = 1;
		int result = 0;
	 ...
	 * @return
	 */
	private String stringToFourByteHexString(String str){
	...
	private  long getUnsignedLong(long value) {
		byte[] buf = new byte[4];
	...
	//	String cid="45010053454d3038471c05efa8e1b191";  // /dNsps01ui7/awS3mYXcqQ==
		...
		ChangeCid  changeCid= new ChangeCid(cid);
		char[] k=changeCid.cidToOtherString().toCharArray();
		
		System.out.println(k);
		System.out.println(md52String32( cid.toCharArray().toString().getBytes() ));
		System.out.println(md52String32( Hex.decodeHex(cid.toCharArray()) ));
		
		byte[] input=Hex.decodeHex(changeCid.cidToOtherString().toCharArray());
	//	System.out.format("%02x", new Object[]{new Integer(16)});
		System.out.println(input);
		cid=(new String(Base64.encode(input)));
